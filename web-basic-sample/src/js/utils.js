 ...
 */
export const analyzeAssassment4 = (message) => {
  if (typeof message === 'string') {
    /**
     * This is the last message sent for this channel.
     * Got it from: channel.lastMessage
     */
    if (message.toLocaleLowerCase().indexOf('sendbird') !== -1) {
      alert('sendbird');
    }    
  } else {
    /**
     * We can also catch the message when the group is opened.
     * 
     * That message is an object and not a string so we need to 
     * get the "message" property for the message object.
     */
    if (message.message.toLocaleLowerCase().indexOf('sendbird') !== -1) {
      alert('sendbird');
    }
  }
}
